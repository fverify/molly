...
(define (get-smt-arith-fn-symb op)
  (cond
    [(equal? op '+) "fp.add"]
    [(equal? op '-) "fp.sub"]
    [(equal? op '*) "fp.mul"]
  ...
  (cond
    [(arithmetic-operator? op);;delete check for rounding mode from here 
     (let* ((lower-power (get-power-of-two-lower lower))
            ...
            (sum-exp (if real-op?
                         (string-append "(define-fun " result-var-str " () (_ FloatingPoint 8 24) (fp.add RNE " (symbol->string var-left) " "
                                        (symbol->string var-right) "))\n"
                                        "\n(assert (= " result-var-str " (" (symbol->string (get-op op)) " " (symbol->string var-left) " " 
                                        (symbol->string var-right) ")))\n")
                         (string-append 
                          "(define-fun " result-var-str " () (_ FloatingPoint 8 24) (" (get-smt-arith-fn-symb op) " RNE " (symbol->string var-left) " "
                                        (symbol->string var-right) "))\n" 
...
(define (get-all-variables-in exp)
  (cond
    ...
    [(named-prop? exp)
      (let ((check (set! PROP-LIST (append PROP-LIST (list (second exp))))))
      (get-all-variables-in (third exp)))]
...
;;
(define (generate-prop-decl prop-list)
  (if (empty? prop-list) 
      ""
      (string-append "(declare-fun " (symbol->string (first prop-list)) " () Bool)\n"
                  (generate-prop-decl (rest prop-list)))))
...
(define (generate-initial-z3-code-var-list var-list lower higher precision rnd-mode mixed-mode?)
  (if (empty? var-list) ""
      (if mixed-mode?
          ...
          (string-append "(declare-fun " (symbol->string (get-var-symbol (first var-list))) " () (_ FloatingPoint 8 24))\n"
                         ";(declare-const " (symbol->string (get-var-symbol (first var-list))) " Real)\n"
    ...
    [(relational-exp? s-expr)
     (let* (
            ...
            (check (set! NAMED-PROP-CONSTRAINTS (string-append NAMED-PROP-CONSTRAINTS "(assert (= "prop-name " " 
                                                 "(" (get-smt-rel-fn-symb (first s-expr))
...
(define (top-level-code-generator s-expr rnd-mode precision type-of-check lower higher var-list-file-name mode input-constant s-expr-formula mixed-mode?)
  (let* (
         ...
         (prop-decl (generate-prop-decl PROP-LIST))
